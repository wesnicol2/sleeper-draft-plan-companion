...
BOARD_ROWS = 32
CRITERIA = ("fills a position the checkpoint is still short of", "matches the checkpoint's lean")
...
def order_columns(counts, needs, strengths=None):
    ...
    def sort_key(position):
        ...
        return (0 if shortfall > 0 else 1, -shortfall if shortfall > 0 else 0,
                strengths.get(position, 0.0), counts.get(position, 0), TIE_BREAK_ORDER.index(position))
...
def ranked_pool(players, taken_ids, limit, adp_index=None):
    ...
    for player_id, player in players.items():
        if player_id in taken_ids or not player.get("active") or player.get("position") not in TRACKED_POSITIONS:
            continue
    ...
    return [{"rank": index, "player_id": player_id,
             "name": player.get("full_name") or f"{player.get('first_name', '')} {player.get('last_name', '')}".strip(),
             "position": player.get("position"), "team": player.get("team"), "age": player.get("age"),
             "rank_source": source, "rank_value": value}
...
def adp_indexes_for(draft_id, players, fresh=False):
    ...
    try:
        ...
        return adp.build_adp_index(records, players), adp.build_consensus_index(records, players), None
...
def _infer_mock_slot(state, picks, username):
    ...
    mine = next((p for p in picks if user_id and str(p.get("picked_by") or "") == user_id and p.get("draft_slot") is not None), None)
    if mine:
...
def _pick_markers(ranked, future_picks):
    ...
    for ordinal, pick_no in enumerate(future_picks, start=1):
        before_rank = next((e["rank"] for e in ranked if e.get("rank_source") == "adp" and e.get("rank_value") >= pick_no), None)
        markers.append({"ordinal": ordinal, "pick_no": pick_no, "before_rank": before_rank, "beyond_board": before_rank is None})
    return markers
...
def _add_strength_context(state, needs, players, consensus_index, parameters, draft_id, fresh):
    ...
    positions_by_player = {pid: p.get("position") for pid, p in players.items()}
    model = strength.summarize_roster(roster, needs, int(state.get("teams") or 12), starters,
                                      consensus_index, positions_by_player, parameters)
    ...
    state["strength_model"] = {"parameters": {"alpha": parameters.alpha, **{f"beta_{p}": v for p, v in parameters.betas.items()}},
                               "starters": starters, "starter_source": starter_source,
                               "targets": model["targets"], "consensus_players_matched": len(consensus_index)}
...
def build_board(draft_id, username=None, fresh=False, strength_parameters=None):
    ...
    try:
        ...
        state.update({"board_error": f"player pool unavailable: {exc}", "columns": order_columns(counts, needs),
                      "ranked": [], "criteria_max": len(CRITERIA), "rows": rows, "decision_context": [],
                      "decision_rules": decision.decision_rules(), "future_pick_markers": []})
        return state
    ...
    strengths = _add_strength_context(state, needs, players, consensus_index, parameters, draft_id, fresh)
    ...
    state["picks_until_my_turn"] = future_picks[0] - current_pick if future_picks and current_pick is not None else None
    lean = (checkpoint or {}).get("lean")
    ranked = ranked_pool(players, taken, rows, rank_index)
    positions_by_player = {pid: p.get("position") for pid, p in players.items()}
    current_model = {"positions": state["positional_strength"], "targets": state["strength_model"]["targets"]}
    ...
    for entry in ranked:
        ...
        entry["strength_if_drafted"] = strength.candidate_strength(state.get("my_roster") or {}, entry, current_model,
            int(state.get("teams") or 12), starters, consensus_index, positions_by_player, parameters)

    state["decision_context"] = decision.build_decision_context(players, taken, rank_index, current_pick, future_picks,
                                                                  needs, [e["player_id"] for e in ranked])
    ...
    state.update({"decision_rules": decision.decision_rules(), "future_pick_markers": _pick_markers(ranked, future_picks),
                  "columns": order_columns(counts, needs, strengths), "ranked": ranked, "criteria_max": len(CRITERIA),
                  "rows": rows, "plan_last_round": plan_module.last_planned_round(plan_module.load_plan())})
    return state
...
def explain_rankings(draft_id, limit=40, fresh=False):
    ...
    players, _fetched_at = sleeper.load_players()
    taken = {pick["player_id"] for pick in draft.get_picks(draft_id, fresh=fresh) if pick.get("player_id")}
    ...
    for entry in ranked:
        ...
        rows.append({"rank": entry["rank"], "name": entry["name"], "position": entry["position"], "team": entry["team"],
                     "rank_source": entry["rank_source"], "rank_value": entry["rank_value"], "adp": adp_index.get(entry["player_id"]),
                     "search_rank": player.get("search_rank"), "ties": counts_by_value[(entry["rank_source"], entry["rank_value"])]})
    from_adp = sum(1 for row in rows if row["rank_source"] == "adp")
    return {"draft_id": draft_id, "scoring": None, "adp_error": adp_error, "adp_players_matched": len(adp_index),
            "shown": len(rows), "ranked_by": {"adp": from_adp, "search_rank": len(rows) - from_adp},
            "note": "rank_source says which value decided the row. CSV ADP uses the rank in resources/adp.csv; search_rank is only the fallback.",
            "rows": rows}
